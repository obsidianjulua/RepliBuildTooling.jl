...
RepliBuild's core is the ABI-aware C/C++ → Julia compiler bridge. This package is everything *around* it: binary/DWARF inspection, Julia code introspection, LLVM IR tooling, benchmarking, and dataset export. It depends on RepliBuild (never the reverse) and consumes the core's public API + build artifacts — it does not participate in a build. Splitting it out keeps the core's dependency surface lean (no DataFrames/CSV/BenchmarkTools in the backend's precompile path).
